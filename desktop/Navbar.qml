...
Item {
    ...
    Process {
        ...
        command: ["bash", "-lc",
            ...
            + "  /^Audio$/                                    {sec=\"audio\"; sub2=\"\"; next}"
            + "  /^[A-Z][a-zA-Z]+$/                           {sec=\"\";      sub2=\"\"; next}"
            + "  /^[[:space:]]*[├└]─[[:space:]]*Sources:/       {sub2=\"sources\"; next}"
            + "  /^[[:space:]]*[├└]─[[:space:]]*Filters:/       {sub2=\"filters\"; next}"
            + "  /^[[:space:]]*[├└]─/                           {sub2=\"\";      next}"
            + "  sec==\"audio\" && (sub2==\"sources\" || sub2==\"filters\") {"
            + "    star=(index($0,\"*\")>0 && index($0,\"*\")<index($0,\".\")) ? 1 : 0;"
            + "    line=$0;"
            + "    sub(/^[ │├─└*]+/, \"\", line);"
            + "    if (sub2==\"filters\" && line !~ /\\[Audio\\//) next;"
            + "    if (match(line, /^([0-9]+)\\. (.+)\\[/, m)) {"
            + "      gsub(/[ \\t]+$/, \"\", m[2]);"
            + "      printf \"%s\\t%s\\t%d\\n\", m[1], m[2], star;"
            + "    }"
