...
// Each shader takes the live omarchy palette as uniforms, so swapping
// `omarchy theme set <name>` recolours the saver mid-flight without
// restart.
//
...
ShellRoot {
    ...
    // omarchy rewrites the palette via atomic rm+mv on theme set, which
    // races inotify on the file itself. Watch the sibling theme.name
    // marker the same way music-wallpaper does and reload on its change.
    FileView {
        id: paletteFile
    ...
    PanelWindow {
        ...
        // Drives every shader's `iTime`. Resets to zero whenever the
        // saver is dismissed so each session starts at t=0.
        property real elapsed: 0
        // Tracks position in the seconds-between-input window. The first
        // input event arms the dismissal; a tiny grace window prevents
        // the still-held activation keypress from immediately closing.
        ...
        Timer {
            ...
            running: root.active
            onTriggered: {
                overlay.elapsed += 0.016;
                overlay.armedFor += 0.016;
                if (overlay.elapsed >= root.cycleSec) {
                    overlay.elapsed = 0;
                    root.shaderIndex = (root.shaderIndex + 1) % root.shaderCount;
                }
            }
            ...
            Repeater {
                ...
                delegate: Loader {
                    ...
                    sourceComponent: ShaderEffect {
                        ...
                        Behavior on opacity { NumberAnimation { duration: root.fadeMs; easing.type: Easing.InOutQuad } }
                        property real  iTime: overlay.elapsed
        ...
        Item {
            ...
            focus: true
            Keys.onPressed: (e) => {
                if (overlay.armedFor < 0.25) { e.accepted = true; return; }
                // Number keys 1..N jump to a specific shader; everything
                // else dismisses. Capped at 9 because that's how many
                // top-row digit keys there are.
